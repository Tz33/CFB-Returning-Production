...
def refresh_materialized_views(session: Session | None = None) -> None:
    """Refresh mv_returning and mv_incoming; they snapshot at creation and go stale after roster loads."""
    ...
    try:
        session.execute(text("REFRESH MATERIALIZED VIEW mv_returning;"))
        session.execute(text("REFRESH MATERIALIZED VIEW mv_incoming;"))
        session.commit()
    finally:
        if manage_session:
            session.close()
