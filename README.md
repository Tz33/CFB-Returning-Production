...
The model predicts each team's SP+ rating from preseason-knowable features (prior SP+ .61, portal-adjusted offensive continuity +11.0, defensive continuity +1.9 (p=.02), new head coach -1.8, recruiting z +3.3). Continuity enters as separate fitted offense/defense features — the old blended index gave offense ~90% weight by the accident of yards-vs-tackles units; the fitted balance lands near 85/15, so the accident was close to right, but now it's a measurement instead of a coincidence. The model converts rating gaps to per-game win probabilities (logistic on 7,769 games: .128/SP+ point, home field +.30, FCS opponents 93.7%), applies Platt recalibration (gamma=.66, fixes tail overconfidence from predicted-rating noise), and sums exact Poisson-binomial win distributions over the real schedule (conference championship games excluded — their matchups encode season outcomes and book totals exclude them; see `model.simulate.drop_ccgs`).

**Backtest (2019, 2022-2025, everything fit strictly pre-season — including fold-specific Platt calibration; CCGs excluded throughout):** MAE 1.69 wins vs 1.71 (raw-returning baseline) and 1.82 (carry-forward); **54.7% against market win totals (228/417, program-clustered bootstrap p=.066, 95% CI .497-.596)** — suggestive, not significant; the per-season rates (model at or above the baseline in 4 of 5 folds) carry the argument. On the portal-divergence subset the model hits **56.2% vs 48.8%** for an otherwise-identical raw-returning model — the portal adjustment still earns its edge where published returning production is most wrong, though n=80 keeps it directional. On identical market-covered team-seasons the model's MAE (1.73) edges the posted totals' own MAE (1.76), winning 4 of 5 seasons — market-grade accuracy from a free-data pipeline. (Earlier drafts quoted 58.1%/p=.001; that figure leaked CCG outcomes into the schedule, pooled Platt calibration across eval folds, and used an IID binomial test — all corrected here.)
